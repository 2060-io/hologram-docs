...
- **[`sandbox.hologram.zone`](https://sandbox.hologram.zone)** — short-lived demos and reference deployments built off [`hologram-sandbox-agent-example`](https://github.com/2060-io/hologram-sandbox-agent-example) so you can fork-and-go.

## Recommended starting point

If you're new and want to ship a verifiable AI agent of your own, the path is:

1. **Fork [`hologram-sandbox-agent-example`](https://github.com/2060-io/hologram-sandbox-agent-example)** — a complete starter agent with Context7 MCP wired up, Avatar-credential authentication, a Helm chart, and a GHA deploy workflow.
2. **(Optionally) Fork [`hologram-sandbox-deps`](https://github.com/2060-io/hologram-sandbox-deps)** — the trust-anchor + Avatar credential issuer that the example agent depends on. Most builders won't need to redeploy these — `sandbox.hologram.zone` already runs them — but the repo exists if you want to operate your own trust hierarchy.
...
|---|---|---|
| [`hologram-sandbox-agent-example`](https://github.com/2060-io/hologram-sandbox-agent-example) | The canonical fork-and-ship starter for an AI agent. Single agent pack, Helm chart, GHA workflow. | You want to build an agent from scratch. |
| [`hologram-sandbox-deps`](https://github.com/2060-io/hologram-sandbox-deps) | The Organization + Avatar issuers that the example agent trusts. Deployed at `{organization,avatar}.sandbox.hologram.zone`. | You need your own trust-anchor / avatar issuer instead of leaning on the public demos. |
| [`hologram-ai-agent`](https://github.com/2060-io/hologram-ai-agent) | The **engine**. The chatbot container that reads an agent pack, talks to a VS Agent, runs the LLM, dispatches MCP tools. Published as `io2060/hologram-ai-agent`. | You want to understand internals, contribute, or fork the chatbot itself. |
...
### `sandbox.hologram.zone` — fork-and-go reference

| Service | Domain | What's interesting |
|---|---|---|
| **Example agent** | `example.sandbox.hologram.zone` | The agent built from `hologram-sandbox-agent-example`. Context7 MCP, Avatar auth, no RBAC. Mirror of what the quickstart deploys. |
| **Organization (demo)** | `organization.sandbox.hologram.zone` | The example's trust anchor. Built from `hologram-sandbox-deps`. |
| **Avatar (demo)** | `avatar.sandbox.hologram.zone` | The example's Avatar issuer. Built from `hologram-sandbox-deps`. |
